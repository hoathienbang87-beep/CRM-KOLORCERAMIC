...
export function managerKpiCustomerSnapshotHtml(viewModel) {
  ...
  return `<section class="kpi-manager-customer" aria-label="Khách hàng liên quan"><div class="kpi-manager-customer-head"><div><b>Khách hàng liên quan</b><span>Thông tin tại thời điểm Event</span></div>${customer.customerId ? `<button class="small" type="button" data-kpi-current-customer="${html(customer.customerId)}">Xem hồ sơ khách hàng hiện tại</button>` : ""}</div><dl><div><dt>Công ty</dt><dd>${value(customer.companyName)}</dd></div><div><dt>Khách hàng</dt><dd>${value(customer.name)}</dd></div><div><dt>Số điện thoại</dt><dd>${value(customer.phone)}</dd></div><div><dt>Địa chỉ</dt><dd>${value(customer.address)}</dd></div></dl></section>`;
}

export function managerKpiEventCardHtml(viewModel, {selectable = false, focused = false, openAction = false} = {}) {
